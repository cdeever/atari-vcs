...
{{< vcsanim scene="asymmetric-pf" caption="The CPU runs ahead of the beam: it writes PF0/PF1/PF2 in HBLANK for the left half, then rewrites all three in the gap before the seam, so the two halves differ — a mountain range on the left, clouds on the right. Watch the cycle count stay well under 76." >}}
