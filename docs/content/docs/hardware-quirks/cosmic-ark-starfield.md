...
Want to see the *actual instructions* that do it? The [Cosmic Ark game-analysis teardown]({{< relref "/docs/game-analysis/cosmicark" >}}) walks the four-instruction recipe — strobe `HMOVE`, wait a counted delay, then poke `HMM0` mid-comb — straight out of a disassembly of the cartridge.
