...
This chapter covers the bit ordering of the three registers (which is *not* uniform — `PF0` uses only its high nibble, and `PF1` runs the opposite direction from `PF0`/`PF2`), reflection vs. repetition, and the technique of rewriting the registers between scanlines to draw shapes that change down the screen — exactly how the Christmas tree in `xmas/xmas.asm` is rendered.
